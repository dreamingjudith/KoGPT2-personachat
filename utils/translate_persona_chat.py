...
def translate_persona_chat(dataset_mod):
    ...
    for entry in dataset_mod[:]:  # 여기 있는 인덱스로 앞에서 몇 개 번역해 넣을 건지 조절. [1001:] 와 같은 형식으로 스크립트 동시 실행
        entry_dict = dict()
        ...
        filename = str(ii)
        write_persona_chat(filename +'.json',entry_dict) #Personality 당 1 파일 만들기

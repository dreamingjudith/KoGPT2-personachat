...
def translate_persona_chat(dataset_mod):
    ...
    for entry in dataset_mod[:]:  # 여기 있는 인덱스로 앞에서 몇 개 번역해 넣을 건지 조절
        ...
        filename = str(ii)
        write_persona_chat(filename +'_valid.json',entry_dict)
...
def main():
    ...
    valid_translated = translate_persona_chat(persona_valid) #translate_persona_chat.py 가 train 이면 이 파일은 valid 번역
